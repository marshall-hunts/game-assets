...
THREE.PointerLockControls = function ( camera, mass, playerHeight, doubleJump, worldObjects, fallthrough ) {

	var scope = this;

	scope.worldObjects = worldObjects;

scope.fallthrough = fallthrough || 0.1
	...
	scope.updateControls = function() {
		...
		if (validDelta) {
			...
			if ( scope.isOnObject === true ) {
				scope.velocity.y = Math.max( scope.intersections.up[0].point.y, scope.intersections.up[0].point.y );
				scope.jumps = 0;
				...
				//If we start to fall through an object
				 if ((this.getPlayer().position.y  < playerHeight) &&
					 scope.downwardsIntersection &&
				 	 ...
				 	 this.getPlayer().position.y += fallthrough;
				 }
			...
			if ( scope.movements.forward) scope.velocity.z -= 400.0 * scope.delta;
			
			if ( scope.movements.backward) scope.velocity.z += 400.0 * scope.delta;
			if ( scope.movements.left) scope.velocity.x -= 400.0 * scope.delta;
			if ( scope.movements.right) scope.velocity.x += 400.0 * scope.delta;
